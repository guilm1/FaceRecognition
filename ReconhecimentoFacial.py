...
print("\nAjustando o classificador ao conjunto de treino com as  funções de kernel linear e rbf:")
t0 = time()
...
y_score_rbf = clf.fit(X_train_pca, y_train).decision_function(X_test_pca)
...
y_score_linear = classifier.fit(X_train_pca, y_train).decision_function(X_test_pca)
...
for i in range(n_classes):
    fpr_rbf[i], tpr_rbf[i], _ = roc_curve(y_roc[:, i], y_score_rbf[:, i])
    fpr_lin[i], tpr_lin[i], _ = roc_curve(y_roc[:, i], y_score_linear[:, i])
    roc_auc_lin[i] = auc(fpr_lin[i], tpr_lin[i])
    roc_auc_rbf[i] = auc(fpr_rbf[i], tpr_rbf[i])

# Compute micro-average ROC curve and ROC area
fpr_rbf["micro"], tpr_rbf["micro"], _ = roc_curve(y_roc.ravel(), y_score_rbf.ravel())
...
# Compute micro-average ROC curve and ROC area
fpr_lin["micro"], tpr_lin["micro"], _ = roc_curve(y_roc.ravel(), y_score_linear.ravel())
...
print(fpr_lin)
print(tpr_lin)
plt.plot(fpr_lin[2], tpr_lin[2], color='darkorange', lw=lw, label='Linear - Curva ROC (area = %0.2f)' % roc_auc_lin[2])
plt.plot(fpr_rbf[2], tpr_rbf[2], color='blue', lw=lw, label='RBF - Curva ROC (area = %0.2f)' % roc_auc_rbf[2])
...
# plt.figure()
# lw = 2
# plt.plot(fpr_rbf[2], tpr_rbf[2], color='darkorange',
#          lw=lw, label='Curva ROC (area = %0.2f)' % roc_auc_rbf[2])
# plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
# plt.xlim([0.0, 1.0])
# plt.ylim([0.0, 1.05])
# plt.xlabel('Taxa Falso Positivo')
# plt.ylabel('Taxa Verdadeiro Positivo')
# plt.title('RBF - Curva ROC - Receiver Operating Characteristic')
# plt.legend(loc="lower right")


# Implementado com RBF e Linear juntos
...
#     #plt.show()
#
# plot_roc_curve(fpr_lin, tpr_lin, roc_auc_lin)
# plot_roc_curve(fpr_rbf, tpr_rbf, roc_auc_rbf)

# #############################################################################
# Avaliação qualitativa das predições usando matplotlib
